...
class ModularIntradayStrategy:
    ...
    def _check_reentry_momentum(self, df, current_idx):
        ...
        # Condition 1: Current close is higher than close `reentry_momentum_lookback` candles ago
        price_increase_over_lookback = df.iloc[current_idx]['close'] > df.iloc[current_idx - self.reentry_momentum_lookback]['close']

        # Condition 2: Minimum number of green candles in the lookback period (including current)
        green_candles_count = 0
        for k in range(self.reentry_momentum_lookback + 1):
            idx_to_check = current_idx - k
            if idx_to_check >= 0:
                if df.iloc[idx_to_check]['close'] > df.iloc[idx_to_check]['open']:
    ...
    def can_reenter(self, current_price, timestamp, current_row, df, current_idx):
        ...
        if self.last_exit_price is None:
            return True # No previous trade, so re-entry is always allowed

        # Block re-entry if last exit was a time-based exit today
        if self.last_exit_reason == "time" and self.last_time_exit_date == timestamp.date():
            return False

        # Apply specific re-entry conditions
        if self.last_entry_price is not None:
            # Condition 1: Price is above previous entry + buffer
            price_above_prev_entry = current_price > (self.last_entry_price + self.reentry_price_buffer)
            if not price_above_prev_entry: return False

            # Condition 2: Short EMA is above Long EMA (bullish trend state)
            if not (current_row['ema_fast'] > current_row['ema_slow']): return False

            # Condition 3: Above VWAP or Supertrend (whichever is in use)
            indicator_bullish_check = False
            if self.use_vwap and current_row['vwap_bull']: indicator_bullish_check = True
            if self.use_supertrend and current_row['supertrend'] == 1: indicator_bullish_check = True
            
            if (self.use_vwap or self.use_supertrend) and not indicator_bullish_check:
                return False

            # Condition 4: Momentum in the last few candles
            if not self._check_reentry_momentum(df, current_idx): return False

            # If all conditions are met, allow re-entry
            return True
    ...
    def run_strategy(self, df):
        ...
        df['ema_bull'] = df['ema_fast'] > df['ema_slow'] # IMPORTANT: Now represents the bullish state
