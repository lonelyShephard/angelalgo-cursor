...
class ModularIntradayStrategy:
    def __init__(self, params=None):
        ...
        self.position_entry_time = None # To track trade duration
        self.tp1_filled = 0.0
        self.tp2_filled = 0.0
    ...
    def enter_position(self, price, timestamp, reason="Buy Signal"):
        ...
        if self.position_size == 0:
            ...
            self.position_entry_time = timestamp  # Record entry time
            self.tp1_filled = 0.0
            self.tp2_filled = 0.0
            self.trailing_active = False
            log = ["ENTRY", timestamp, f"{price:.2f}", f"{self.position_size:.2f}", reason]
    ...
    def exit_position(self, price, timestamp, qty_percent=100, reason="Exit"):
        ...
        if self.position_size > 0:
            ...
            trade = {
                'entry_time': self.position_entry_time, # Use stored entry time
                'exit_time': timestamp,
            ...
            # Track last exit for re-entry logic and reset position
            if self.position_size <= 0.001: # Use a small threshold for float comparison
                self.last_exit_price = price
                self.last_entry_price = self.position_entry_price
                self.position_size = 0
                self.position_entry_time = None # Reset entry time
                self.tp1_filled = 0.0
                self.tp2_filled = 0.0
                self.trailing_active = False
...
if __name__ == "__main__":
    # Load your CSV
    import os
    csv_path = os.path.join(os.path.dirname(__file__), "NIFTY19JUN2524500PE_ONE_MINUTE.csv")
    df = pd.read_csv(
        csv_path,
    ...
    if "error" not in results:
        ...
        if len(results['trades_df']) > 0:
            print("\n=== SAMPLE TRADES ===")
            # Updated to show trade duration
            print(tabulate(results['trades_df'][['entry_price', 'exit_price', 'pnl', 'trade_duration', 'reason']].head(), headers='keys', tablefmt='github'))
    else:
